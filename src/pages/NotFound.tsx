...
export default function ErrorPage({ code = 404, title, message }: ErrorPageProps) {
  ...
  // Informasi default untuk kode error umum yang ditangani.
  // Tambahkan kode error baru di dalam objek ini jika dibutuhkan.
  const errorData = {
    404: {
  ...
  // Pencadangan (fallback) jika kode error yang diterima di luar daftar yang ditentukan.
    ...
    <div className="h-screen w-full flex items-center justify-center p-6 bg-white overflow-hidden">
      <div className="relative max-w-2xl w-full text-center">
        ...
        >
          ...
          {/* Rekomendasi Tautan Pencarian Cepat */}
          <div className="mt-16 pt-8 border-t border-gray-50">
             ...
             <div className="flex flex-wrap justify-center gap-3">
                {['Katalog Buku', 'Berita Resmi', 'Profil Dinas', 'Pojok Carita']
                  .filter(tag => tag !== 'Katalog Buku' || SITE_CONFIG.FEATURES.ENABLE_CATALOG)
                  .map(tag => (
                    // Semua rekomendasi rute non-katalog diarahkan ke halaman artikel.
                    <Link 
                      key={tag} 
