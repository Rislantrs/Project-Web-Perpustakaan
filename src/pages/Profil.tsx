...
import {
  ...
  Mail, Phone, MapPin, User, AlertCircle, LogOut
} from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { getCurrentUser, isLoggedIn, updateMember, logout, getInitials, type Member } from '../services/authService';
import { getMemberBorrows, getMemberQueues, type BorrowRecord, type QueueRecord } from '../services/bookService';
...
export default function Profil() {
  ...
  const [activeTab, setActiveTab] = useState<'statistik' | 'riwayat' | 'antrian'>('statistik');
    ...
    <div className="bg-[#f0f2f5] min-h-screen pb-20">
      ...
      <div className="relative h-52 bg-gradient-to-br from-[#0c2f3d] via-[#1a4254] to-[#8b1c24] overflow-hidden">
        ...
        <div className="absolute top-4 right-4">
          <button onClick={handleLogout} className="inline-flex items-center gap-2 text-white/80 hover:text-white text-sm bg-black/20 backdrop-blur-sm px-3 py-2 rounded-xl transition-colors">
          ...
          <div className="p-5">
            ...
            {activeTab === 'statistik' && (
              <div className="space-y-4">
                {[
                  { icon: User, label: 'Nama Lengkap', value: user.namaLengkap },
                  { icon: Mail, label: 'Email', value: user.email },
                  { icon: Phone, label: 'Telepon', value: user.telepon || '-' },
                  { icon: MapPin, label: 'Alamat', value: user.alamat || '-' },
                  { icon: Calendar, label: 'Tanggal Lahir', value: user.tanggalLahir || '-' },
                  { icon: Users, label: 'Jenis Kelamin', value: user.jenisKelamin === 'L' ? 'Laki-laki' : 'Perempuan' },
                ].map(item => (
